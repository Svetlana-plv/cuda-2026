...
// потоки внутри 1 блока выполняются на 1 мультипроцессоре
// могут взаимодействовать:
// 1) через разделяему память
// 2) точки синхронизации ( ??? )
// два потока различных блоках - только через глобальную память
// 
// каждый блок целиком выполняется на 1 мультипроцессоре
// если ресурсы позволяют - несколько блоков могут "одновременно"
// исполняться на одном мультипроцессоре
// 
// все потоки выполняются на одном мультипроцессоре, 
// группируются в варпы, в варп попадают потоки с последов. ID
// варп == 32 подряд идущих потока
// выполнение - производится варпами
//  скалярные процессоры данного мультипроцессра параллельно выполняют
// одну и ту же инструкцию для всех потоков варпа (SIMT)
// 
// скалярные процессоры разных мультипроцессоров независимы
// 
// схема организации вычислений:
// задача разбивается на  независимые подзадачи, 
// каждая из которых решается параллельно одним блоком
// 
// хост == CPU
// девайс ==  GPU
// ядро == подпрограмма, параллельно выполняемая потоками на девайсе
// 
// 
// 
//
